...
- **Hike Club API integration** (`HikeAPI.swift`, `Keychain.swift`, `SettingsView.swift`) — the app's *only* network call, and the sole exception to "no network". A `Hike` optionally stores `apiHikeID` (set in `NewHikeSheet` or while `planned` in `HikeDetailView`); `HikeAPI.fetch(id:)` does a read-only `GET /hike/{id}` (`x-api-key` header) against a Worker API and returns `HikeResponse` (meeting point, trails, expiring map URL, weather). The fetched data is **ephemeral view `@State`, never persisted** (weather is time-sensitive, the map URL expires) — only `apiHikeID` is stored. Config: base URL in `UserDefaults` (`@AppStorage`), API key in the **Keychain** (`kSecAttrAccessibleWhenUnlockedThisDeviceOnly`, never in git/binary), both entered in the **Settings** tab. Security invariants to preserve: base URL is `https`-only (rejected otherwise in `SettingsView`, so the key can't leak to an http/wrong host); `apiHikeID` is validated + `URL.appending(path:)`-encoded (no path escaping); remote strings render as `Text(variable)` and response URLs are `https`-gated before `AsyncImage`/`Link` (no markdown/deep-link injection); no ATS exception. Like `HealthImport`, the pure decode (`HikeResponse.decode`) is unit-tested (`HikeAPITests`) while the `URLSession`/Keychain calls are device-only and untested. New `.swift` files auto-join the target via the project's file-system-synchronized groups — no `project.pbxproj` edit needed.
